// Variables globales - se inicializan solo si los elementos existen
...
function mostrarObras() {
    ...
    if (totalObras) totalObras.textContent = obras.length;
}
...
function abrirModalDetalle(i) {
    ...
    if (modalTitle) modalTitle.textContent = `Detalle de: ${obra.nombre}`;
    if (modalBody) {
        modalBody.innerHTML = `
...
// Solo ejecutar si estamos en la página con formulario
if (formulario) {
...
// Solo mostrar obras si el contenedor existe
if (listaObras) {
    mostrarObras();
}
